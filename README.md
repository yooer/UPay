...
项目已实现后台服务命令行一键管理，支持后台常驻、退出、重启及状态监测：
...
- **后台启动**：
  ...
  ./UPay --daemon
  ```
  启动后，系统自动在后台执行自守护，主守护进程 PID 将被写入当前目录下的 `upay.pid`。
...
- **关闭服务**：
  ...
  ./UPay stop
  ```
  优雅并彻底地终止后台守护进程以及所有工作子进程，同时自动清理 `upay.pid`。

- **重启服务**：
  ```bash
  ./UPay restart
  ```
  自动终止当前运行的后台服务并自动重新拉起新的后台实例。

- **前台调试启动**：
  ```bash
  ./UPay
  ```
  直接执行即在前台运行，日志实时打印至控制台，适用于 Docker 容器环境或本地调试。

### 日志系统说明

系统运行日志均收纳于 `logs/` 文件夹下，包含两类不同用途的日志：

1. **`logs/upay.log` (核心业务日志)**：
   由应用程序代码中配置的 zap 组件输出，用于记录数据库交互、接收订单创建、轮询支付验证以及通知回调等核心业务状态。
2. **`logs/upay_daemon.log` (控制台输出日志)**：
   仅在后台模式（`start`/`-d`/`--daemon`）运行时产生。该文件为程序 `stdout`/`stderr` 的物理重定向，用于捕获 Gin 路由映射日志，以及 Go 运行时未捕获的 Panic 崩溃栈，方便低层系统故障排查。
